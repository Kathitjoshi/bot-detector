...
class DataProcessor:
    ...
    def create_sample_dataset(self, n_samples: int = 10000, 
                             bot_ratio: float = 0.3) -> pd.DataFrame:
        """
        ...
        humans = pd.DataFrame({
            ...
            'default_profile_image': np.random.choice([0, 1], n_humans, p=[0.95, 0.05]),
    ...
    def clean_data(self) -> pd.DataFrame:
        ...
        """
        if self.df is None:
            raise ValueError("No data loaded. Call load_data() or create_sample_dataset() first")
