...
class FeatureEngineer:
    ...
    def create_basic_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Follower ratios
        df["follower_following_ratio"] = df["followers_count"] / (
            df["following_count"] + 1
        )
        df["following_follower_ratio"] = df["following_count"] / (
    ...
    def create_crypto_features(
        self, df: pd.DataFrame, content_column: str = "tweet_text"
    ) -> pd.DataFrame:
        """
    ...
    def detect_bot_network(
        self, user_ids: List[str], content_hashes: List[str], time_threshold: int = 3600
    ) -> Dict[str, List[str]]:
        """
        ...
        networks = {h: users for h, users in hash_to_users.items() if len(users) > 1}
