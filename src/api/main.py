...
async def lifespan(app: FastAPI):
    ...
    model_path = Path(__file__).parent.parent.parent / "models" / "saved_models" / "best_model.joblib"
...
class UserData(BaseModel):
    ...
    default_profile_image: int = Field(0, ge=0, le=1, description="Using default image (0 or 1)")
    geo_enabled: int = Field(0, ge=0, le=1, description="Geo tagging enabled (0 or 1)")
    description_length: int = Field(0, ge=0, description="Length of profile description")
    avg_tweets_per_day: float = Field(..., ge=0, description="Average tweets per day")
    avg_retweet_ratio: float = Field(..., ge=0, le=1, description="Average retweet ratio")
...
async def predict_bot(user: UserData):
    ...
    try:
        ...
        message = "This account shows bot-like behavior" if is_bot else "This account appears to be human"
...
async def model_info():
    ...
    return {
        ...
        "feature_importance_available": model_data.get('feature_importance') is not None
    }
